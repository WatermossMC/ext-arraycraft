...
--SKIPIF--
<?php 
if(!extension_loaded("arraycraft")) die("skip extension not loaded");
?>
--FILE--
<?php
...
for($i = 0; $i < 20; ++$i){
	try{
		var_dump(\watermossmc\world\format\PalettedBlockArray::getExpectedWordArraySize($i));
...
try{
	var_dump(\watermossmc\world\format\PalettedBlockArray::getExpectedWordArraySize(257));
}catch(\InvalidArgumentException $e){
	echo $e->getMessage() . "\n";
}
var_dump("ok");
