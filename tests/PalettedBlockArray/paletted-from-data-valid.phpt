...
foreach([1,2,3,4,5,6,8,16] as $bitsPerBlock){
	...
	$palette = \watermossmc\world\format\PalettedBlockArray::fromData($bitsPerBlock, str_repeat("\x00", $payloadSize), [$bitsPerBlock]);
	var_dump($palette->get(0, 0, 0));

	try{
		$palette = \watermossmc\world\format\PalettedBlockArray::fromData($bitsPerBlock, str_repeat("\x00", $payloadSize - 1), [$bitsPerBlock]);
		var_dump($palette->get(0, 0, 0));
	...
	try{
		$palette = \watermossmc\world\format\PalettedBlockArray::fromData($bitsPerBlock, "", [$bitsPerBlock]);
		var_dump($palette->get(0, 0, 0));
	}catch(\RuntimeException $e){
