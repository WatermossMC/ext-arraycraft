...
foreach([-1, PHP_INT_MIN, PHP_INT_MAX, 8, 9] as $offset){
	try{
		\watermossmc\world\format\io\SubChunkConverter::convertSubChunkFromLegacyColumn($ids, $data, $offset);
...
for($y = 0; $y < 8; ++$y){
	try{
		\watermossmc\world\format\io\SubChunkConverter::convertSubChunkFromLegacyColumn($ids, $data, $y);
		var_dump("ok: $y");
	}catch(\InvalidArgumentException $e){
